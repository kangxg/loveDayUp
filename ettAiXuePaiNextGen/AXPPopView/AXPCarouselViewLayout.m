...
@interface AXPCarouselViewLayout () {
    CGFloat _viewHeight;
    CGFloat _itemHeight;
}
...
@implementation AXPCarouselViewLayout
...
- (void)prepareLayout {
    ...
    dispatch_once(&onceToken, ^{
        self.collectionView.contentOffset = CGPointMake(-540 + (self.selectedIndex)*286 + 143, 0);
    });
}
...
- (CGPoint)targetContentOffsetForProposedContentOffset:(CGPoint)proposedContentOffset withScrollingVelocity:(CGPoint)velocity {
    ...
    AXPWhiteboardManagerCollectionView *collectionView = (AXPWhiteboardManagerCollectionView *)self.collectionView;
    collectionView.currentSelectedIndex = index;
    collectionView.isScroll = YES;
    [collectionView reloadData];
    
    NSLog(@"1-----index%f proposedContentOffset.x :%f",index,proposedContentOffset.x);
