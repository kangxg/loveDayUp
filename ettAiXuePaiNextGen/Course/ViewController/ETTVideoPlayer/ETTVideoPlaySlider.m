...
@interface ETTVideoPlaySlider ()

@property (nonatomic,strong) UIImageView *thumb;

@end

@implementation ETTVideoPlaySlider

- (instancetype)initWithFrame:(CGRect)frame {
    self                 = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, CGRectGetWidth(frame), 30)];
    [self addSubview:self.thumb];
    self.backgroundColor = [UIColor clearColor];
    self.minTrackColor   = [UIColor colorWithRed:0.02 green:0.47 blue:0.98 alpha:1];//播放后颜色
    self.maxTrackColor   = [UIColor whiteColor];//没有缓冲之前颜色
    self.bufferTrackColor = [UIColor lightGrayColor];//缓冲颜色
    return self;
}
