...
- (instancetype)init {
    ...
    if (self) {
        ...
        _displayMode = ETTVideoPlayerDisplayModeAspectFit;
...
- (void)playInContainer:(UIView *)container {
    ...
    switch (self.displayMode) {
        case ETTVideoPlayerDisplayModeAspectFit:
...
- (void)setDisplayMode:(ETTVideoPlayerDisplayMode)displayMode {
...
- (void)setPath:(NSString *)path {
    ...
    _path = path;
    if (self.playState == ETTVideoPlayerPlayStatePlaying) {
        [self.player pause];
    }
    ...
    __weak ETTVideoPlayer *weakSelf = self;
    self.observer = [self.player addPeriodicTimeObserverForInterval:CMTimeMake(1.0, 1.0) queue:dispatch_get_global_queue(0, 0) usingBlock:^(CMTime time) {
        ...
        if (weakSelf.currentTimeBlock || weakSelf.totalTimeBlock) {
            
            weakSelf.currentTimeBlock(f);
            weakSelf.totalTimeBlock(max);
        }
...
- (ETTVideoPlayerPlayState)playState {
    if (ABS(self.player.rate - 1) <= 0.000001) {
        return ETTVideoPlayerPlayStatePlaying;
    }
    return ETTVideoPlayerPlayStateStop;
}
