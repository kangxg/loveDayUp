...
- (void)viewDidLoad
{
    ...
    mainToolbar.backgroundColor = [UIColor colorWithRed:38/255.0 green:147/255.0 blue:220/255.0 alpha:1.0];
    
    mainToolbar.titleLabel.text = self.navigationTitle;
...
{   
    [ETTCoursewarePresentViewControllerManager sharedManager].thumViewController = nil;
...
#pragma mark 上部缩略图的某页
- (void)thumbsView:(ReaderThumbsView *)thumbsView didSelectThumbWithIndex:(NSInteger)index
{
	NSInteger page = (showBookmarked ? [[bookmarked objectAtIndex:index] integerValue] : (index + 1));
    
    [[NSNotificationCenter defaultCenter]postNotificationName:@"clickTopThumbnailPage" object:[NSString stringWithFormat:@"%ld",page]];
...
{
	CGRect iconRect = bookMark.frame;
    
    iconRect.origin.y = (-2.0f);
...
- (instancetype)initWithFrame:(CGRect)frame
{
	if ((self = [super initWithFrame:frame]))
	{
		...
		defaultRect.size.width = newWidth;
        
        defaultRect.origin.x += offsetX;
...
- (void)reuse
{
    ...
    tintView.frame = imageView.bounds; backView.frame = defaultRect;
