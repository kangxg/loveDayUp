...
- (void)updateMinimumMaximumZoom
{
    ...
    self.maximumZoomScale = (zoomScale * ZOOM_MAXIMUM);//最大缩放比例
...
- (instancetype)initWithFrame:(CGRect)frame fileURL:(NSURL *)fileURL page:(NSUInteger)page password:(NSString *)phrase
{
	if ((self = [super initWithFrame:frame]))
	{
		...
		if (theContentPage != nil) // Must have a valid and initialized content page
		{
            ...
            //theContentPage被添加到theContainerView里面
...
//是否有阴影
            ...
            //theContainerView被添加到contentView上面
...
{
	CGFloat zoomScale = self.zoomScale; // Current zoom 当前缩放

	CGPoint point = [recognizer locationInView:theContentPage];

	if (zoomScale < self.maximumZoomScale) // Zoom in  当前缩放小于最大缩放
	{
	...
	else // Handle fully zoomed in 大于或等于缩放比例
	{
...
- (void)zoomDecrement:(UITapGestureRecognizer *)recognizer
{
	...
	{
		zoomScale /= ZOOM_FACTOR; // Zoom out by zoom factor amount根据缩放比例进行缩小
